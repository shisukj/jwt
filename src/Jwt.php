...
class Jwt
{
    ...
    public function createToken($payload, $type = Jwt::SCOPE_TOKEN)
    {
        if ($payload instanceof JwtBuilder) {
            ...
            switch ($type) {
                ...
                case self::SCOPE_REFRESH:
                    $jwtObj->setRefreshExpiration($time + $this->refreshTtl);
                    break;
            }
        } else {
        ...
        if ($this->loginType == 'sso' && $jwtObj->getAudience() == '') {
            throw new JWTException("There is no Audience key in the claims", 400);
        }
    ...
    public function verifyToken(string $Token)
    {
        ...
        try {
            ...
            //签名验证
            if (self::signature($base64header . '.' . $base64payload, $this->secret, $base64deadheaded['alg']) !== $sign) throw new TokenValidException('token签名错误', 400);
            $payload = json_decode(self::base64UrlDecode($base64payload), true);
        } catch (\Throwable $e) {
            throw new TokenValidException('token解析无效', 401);
        }
        switch (true) {
    ...
    public function verifyRefreshToken(string $Token)
    {
        ...
        try {
            ...
            if (empty($base64deadheaded['alg'])) throw new TokenValidException('token 错误', 401);
            //签名验证
            if (self::signature($base64header . '.' . $base64payload, $this->secret, $base64deadheaded['alg']) !== $sign) throw new TokenValidException('token签名错误', 400);//500
            $payload = json_decode(self::base64UrlDecode($base64payload), true);
        } catch (\Throwable $e) {
            throw new TokenValidException('token解析无效', 400);//500
        }
        switch (true) {
